#!/usr/bin/env python3
"""Build the self-describing Fast stage v3 core+extension bundle."""
...
def main() -> int:
    ...
    if core[3:7] != MAGIC or core[7:9] != b"\x01\x02":
        raise ValueError("fastboot v3 core metadata is missing or malformed")
    if len(core) > CORE_SIZE:
        raise ValueError(f"fastboot v3 core is {len(core)} bytes, limit {CORE_SIZE}")
    if len(extension) > EXTENSION_SIZE:
        raise ValueError(
            f"fastboot v3 extension is {len(extension)} bytes, "
    ...
    print(
        f"Fastboot v3 bundle: core={len(core)}/{CORE_SIZE}, "
